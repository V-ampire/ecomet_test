...
class GithubReposScrapper:
    ...
    async def _make_request(self, endpoint: str, method: str = "GET", params: dict[str, Any] | None = None) -> Any:
        async with self.semaphore:
            async with self.rate_limit:
                logger.debug(f"_make_request: {endpoint}, {params=}")
    ...
    async def get_repositories(self) -> list[Repository]:
        try:
            ...
            logger.error(f"Fail to get top repositories, {str_exception(exc)}")
            return []
        logger.info(f"Requested top repost {len(top_repos_map.values())}")
